...
export default class Addons extends React.Component {
  ...
  openAddForm() {
    this.props.router.push(`${URL_CONFIG.addons_path}/new`);
  }
          ...
          <PanelBody>
            <Grid>
              ...
              <Row className="boat-amenity-tbl">
                <Col md={12}>
                  <DatatableComponent ajaxUrl={URL_CONFIG.addons_path} />
                </Col>
              </Row>
            </Grid>
          </PanelBody>
...
class DatatableComponent extends React.Component {
  ...
  removeFn() {
    ...
    $.ajax({
      url: `${URL_CONFIG.addons_path}/${this.addonId}`,
      method: "DELETE"
    }).then(
      () => {
  ...
  openConfirmModal(addonId) {
    this.addonId = addonId;
    this.confirmDeleteModal.open();
  }

  goToEdit(addonId) {
    this.props.router.push(`${URL_CONFIG.addons_path}/${addonId}/edit`);
  }
      ...
      <Table ref={c => (this.datatable = c)} className="display" cellSpacing="0" width="100%">
        <thead>
          <tr>
            <th>ID</th>
            <th>Name</th>
            <th data-searchable="false">Price</th>
            <th data-searchable="false">Quantity</th>
